...
async def answer(event):
    ...
    if 'gemini AI' in event.raw_text:
        query = event.raw_text[-3].replace('!ai', '').replace('!ии', '').replace('@afganor', '').replace('!aiprompt', '')
    else:
    ...
    try:
        ...
        return
    for line in res:
        await event.client.send_message(event.chat.id, line + '\n\n gemini AI\n' + str(len(context)-1) +'/60', reply_to=event.message.id, parse_mode='md')
