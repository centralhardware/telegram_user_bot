...
async def top(event):
    ...
    res = clickhouse.query("""
            ...
            from (select any(user_id), count(*) as count
                from chats_log
                where chat_id=-1001633660171 and has(tokens(lowerUTF8(message)), %(word)s) and message not like '!top %'
                group by user_id
                limit 10
                UNION ALL
                select 428985392, count(*) as count
                from telegram_messages_new
                where id=-1001633660171 and has(tokens(lowerUTF8(message)), %(word)s) and message not like '!top %') 
            order by count desc 
    """, {"word": word})
    msg = ""
